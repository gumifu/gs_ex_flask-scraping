...
function handleStartButtonClick() {
    ...
    recognition.onresult = event => {
        ...
        } else if (text.indexOf('デザイン') !== -1) {
            // デザインだったら、API経由でおすすめ記事を取得する.
            showRecommendDesign();
        } else if (text.indexOf('建築') !== -1) {
            // 建築だったら、API経由でおすすめ記事を取得する.
            showRecommendArchitecture();
        } else if (text.indexOf('アート') !== -1) {
            // アートだったら、API経由でおすすめ記事を取得する.
            showRecommendArt();
        } else {
            // ニュース以外はわからないよ〜.
            let synthes = new SpeechSynthesisUtterance('ごめんなさい,わかりません');
            synthes.lang = "ja-JP";
...
function showRecommendDesign() {

    api('/api/recommend_design').then(response => {
        let { content, link } = JSON.parse(response);
        console.log(content);
...
function showRecommendArchitecture() {

    api('/api/recommend_architecture').then(response => {
        let { content, link } = JSON.parse(response);
        console.log(content);
...
function showRecommendArt() {

    api('/api/recommend_art').then(response => {
        let { content, link } = JSON.parse(response);
        console.log(content);
        ...
        document.getElementById('text').innerHTML = `<a href="https://bijutsutecho.com${link}">${content}</a>`;
    });
}
...
function startIntro() {
    ...
    return new Promise((resolve, reject) => {
        ...
        let texts = "ニュース,建築,デザイン,アート いづれか聞いてみてください".split('');
